...
folderPathTrain = "gesture/train/20"
folderPathTest = "gesture/test/20"
...
while True:
    ...
    if num_frames < 60:
        ...
        if num_frames <= 59:
            cv2.putText(frame_copy, "Arrumando o Fundo...", (80, 400), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 0, 255), 2)
        ...
        if hand is not None:
            ...
            if num_imgs_taken <= 1000:
                cv2.imwrite(folderPathTrain+"\\" + str(num_imgs_taken) + '.jpg', thresholded)
            elif num_imgs_taken <= 1100:
                cv2.imwrite(folderPathTest + "\\" + str(num_imgs_taken) + '.jpg', thresholded)
            else:
                break
            num_imgs_taken +=1
        else:
